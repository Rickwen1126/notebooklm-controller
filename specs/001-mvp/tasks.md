...
## Review 1.5 Hotfix — Tab Pool + Notebook-First 架構對齊修正

**Background**: Review Point 1.5 code alignment 完成後，透過 Sky Eye Tour 03 review + Code Review tour 發現的問題。分三層處理：Bug（必修）、Architecture（應修，Phase 6 前）、Tech Debt（可 defer）。

**來源**:
- Tour 03 review Finding A-D（使用者 review 標註）
- Code Review tour `review-tab-pool-alignment-20260314.tour`（4 suggestions）
- Tour 02 step-by-step review（waitForTask gap）
...
## Spike 回灌 2（Browser Capability Spike Phase A-F 結論）

**Background**: Browser Capability Spike 全部完成後，以下 finding 尚未回灌到主線 code。spec.md 和 plan.md 已更新（FR-185~188, download infra, CDP Ctrl+A, prompt 零留白），此處追蹤實作 task。
...
- [ ] T-SB04 [P] Unit tests for download setup in `tests/unit/tab-manager/tab-manager.test.ts` (CDP Browser.setDownloadBehavior called on tab creation, downloadPath config)
- [ ] T-SB05 Implement download behavior setup in `src/tab-manager/tab-manager.ts` — 新 tab 建立時透過 CDP `Browser.setDownloadBehavior` 設定 `{ behavior: "allowAndName", downloadPath: config.downloadDir }`。預設下載目錄 `~/.nbctl/downloads/`，加入 `config.ts`。

### CDP Ctrl+A 修正（Finding #43, cdp-helpers）

- [ ] T-SB06 [P] Unit tests for selectAll in `tests/unit/tab-manager/cdp-helpers.test.ts` (selectAll uses JS document.activeElement.select(), not CDP key event)
- [ ] T-SB07 Implement selectAll fix in `src/tab-manager/cdp-helpers.ts` — 新增或修正 selectAll helper：用 `page.evaluate(() => { const el = document.activeElement; if (el && typeof el.select === 'function') el.select(); })` 取代 CDP `Input.dispatchKeyEvent` 的 Ctrl+A。確保 Angular Material dialog 等場景可靠。
...
| Phase 14 (Polish) | 4 |
| MVP tasks (Phases 1-7, incl. 5.5 + hotfix + spike) | 113 |
