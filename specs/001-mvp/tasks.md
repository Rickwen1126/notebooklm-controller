...
**🔍 Review Point 1**: 開發者主動發起 `/reviewCode` + `/codetour`（Constitution X）。地基層，所有 user story 依賴它。
...
**🔍 Review Point 2**: 開發者主動發起 `/audit` + `/codetour` + `/reviewCode`（Constitution X）。MVP 整體架構穩定，適合全面審查。
...
**🔍 Review Point 3**: 開發者主動發起 `/audit` + `/codetour` + `/reviewCode`（Constitution X）。全部完成，production readiness 審查。
...
4. Commit after each task or logical group, MUST pass lint + test (Constitution X)
...
| Phase 13 US15+US19+US23+US24 | 4 |
| Phase 14 (Polish) | 4 |
| MVP tasks (Phases 1-7) | 84 |
| Post-MVP tasks (Phases 8-14) | 29 |
