...
TabPool (runtime, managed by TabManager)
  └── manages → TabHandle (pooled: active | idle)

NetworkGate (runtime)
  └── has one → NetworkHealth

AgentSession (runtime, per task)
  ├── uses one → TabHandle (via TabPool.acquireTab())
...
type NotebookStatus =
  | "ready"       // 已註冊且可操作（exec 時系統自動取得 tab）
  | "operating"   // 正在執行操作（已佔用 tab）
...
**Purpose**: TabManager tab pool 中的 tab 句柄（CDP session），可被 acquire/release 重用。

```typescript
interface TabHandle {
  tabId: string;                       // 唯一識別碼（UUID）
  notebookAlias: string;               // 目前（或最後）操作的 notebook
  url: string;                         // Navigate 的目標 URL
  state: "active" | "idle";           // active = 正在被 task 使用，idle = 可被回收重用
  acquiredAt: string;                  // ISO 8601 最近一次取得時間
  releasedAt: string | null;          // ISO 8601 最近一次歸還時間（idle 時）
...
**設計說明**：Tab 是操作的執行單位——截圖/DOM 操控需要獨立 CDP session，
因此操作期間 notebook 獨佔一個 tab。完成後 tab 歸還 pool（標記為 idle），
可被其他 notebook 的操作回收重用。這是操作期間的獨佔，不是永久 1:1 綁定。
Agent 透過 CDP 底層 API（`Input.dispatchMouseEvent`、`Page.captureScreenshot`）操作，
...
> 原設計為 browser tool factory 的最小介面。經 spike 驗證後決定直接使用 TabHandle
> （已含 cdpSession + page），不需要額外抽象。Tool 實作從 spike 複製並適配 TabHandle。
> Spike playground 保持獨立，後續 repair 機制再處理 single source of truth。

### UIMap / UIMapElement

**Purpose**: Locale-specific UI 元素對應表，供 NOTEBOOKLM_KNOWLEDGE template 使用。

```typescript
interface UIMapElement {
  text: string;                              // UI 上顯示的文字
  match?: "text" | "placeholder" | "aria-label"; // 匹配方式（預設 text）
  disambiguate?: string;                     // 消歧規則（如 "y > 400"）
}

interface UIMap {
  locale: string;                            // e.g. "zh-TW"
  verified: boolean;                         // 是否經過 smoke test 驗證
  elements: Record<string, UIMapElement>;    // UI 元素表（create_notebook, paste_source_type, ...）
  selectors: Record<string, string>;         // CSS selectors（answer, question, suggestions, ...）
}
```

**來源**：`src/config/ui-maps/{zh-TW,en,zh-CN}.json`（內建，隨 npm 發布）。
Daemon 啟動時偵測 Chrome locale → `resolveLocale()` → `loadUIMap()` → 注入 agent KNOWLEDGE template。
...
interface DaemonStatusResult {
  running: boolean;
  tabPool: {
    usedSlots: number;                 // 正在使用中的 tab 數量（active）
    maxSlots: number;                  // Pool 上限（預設 10）
    idleSlots: number;                 // 可回收重用的 idle tab 數量
  };
  network: NetworkHealth;
  activeNotebooks: string[];           // 正在執行操作的 notebook alias 列表
