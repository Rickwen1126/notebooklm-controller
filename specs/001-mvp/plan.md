...
Scheduler.dispatch(task)
  → SessionRunner.run(task, tabHandle, agentConfig)
    → client.createSession({ model: "gpt-4.1", tools: buildToolsForTab(tabHandle), customAgents: agentConfigs, systemMessage: routingPrompt, hooks })
...
**兩層 Agent 架構**（Spike 1 驗證後確認）：
- **Main Agent**（路由層）：session systemMessage 只放輕量路由指令（可用 agents 列表 + 描述）。不含 UI 知識。
- **Subagent**（執行層）：每個 `agents/*.md` = 一個 focused spike session。Prompt 引用 `{{NOTEBOOKLM_KNOWLEDGE}}`，agent-loader 從 UI map config 注入 locale-specific 內容。
