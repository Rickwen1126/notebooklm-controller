...
3. **Given** daemon 正在執行，
   ...
   **Then** 回傳 daemon 級別狀態：
   ```json
   { "running": true, "googleSessionValid": true,
     "network": "healthy",
...
指令語意：
- `register_notebook` tool（url, alias）：將既有 NotebookLM notebook 納入管理（使用者已知 URL）。
- `register_all_notebooks` tool：以交互式方式批次納管使用者帳號中的所有既有 notebook。
...
**Independent Test**: register_notebook 納管兩個 notebook，同時對兩個 notebook 發出 exec 操作，
驗證 parallel 執行（各自取得獨立 tab）。list_notebooks 列出所有。remove_notebook 移除一個。
另外：使用 register_all_notebooks tool 批次納管。
...
3. **Given** 已註冊多個 notebook，
   ...
   **Then** 回傳 JSON 陣列，每個 notebook 包含 description 與狀態：
   ```json
   [
     { "id": "research", "url": "...", "status": "ready",
       "description": "包含專案認證模組與 API 文件的開發筆記" },
     { "id": "ml-papers", "url": "...", "status": "ready",
       "description": "..." }
   ]
   ```
...
9. **Given** 使用者在 `register_all_notebooks` 過程中想跳過某些 notebook，
...
- **FR-033**: `register_notebook` tool MUST 導航至 notebook URL、掃描其來源清單與標題、
  同步到 local cache。
- **FR-034**: 系統 MUST 提供 `register_all_notebooks` MCP tool，
  以交互式方式批次納管使用者帳號中的所有 notebook。
- **FR-035**: `register_all_notebooks` MUST 依序展示每個 notebook 的標題與 URL，
  讓使用者選擇是否納管並指定別名。
...
- **FR-045**: 系統 MUST 在 register_notebook 後，由 agent 根據 notebook
...
- **FR-053**: _(已移除——`open_notebook` 已砍掉。註冊新 notebook 使用 `register_notebook` tool。)_
...
- **Agent Session**：Per-task 的 AI agent。系統自動透過 TabManager `acquireTab(notebookUrl)`
  ...
  Executor 啟動前系統做 O(1) 錨點判斷（tab.url exact match），結果作為 hint 注入 prompt，
...
- **Agent Config**：參數化的 agent 操作定義（YAML frontmatter + Markdown prompt → SDK 的 `CustomAgentConfig`）。
  包含 prompt template、所需 tool 清單、操作依賴宣告、`startPage`（homepage 或 notebook）。
...
- **Two-Session Planner+Executor 架構**（Phase F Spike 驗證後確認，取代原 CustomAgent sub-agent 方案——
  CustomAgent 無法存取 `defineTool()` custom tools，Finding #39）：
  - **Planner Session**（意圖解析層）：無 browser tools，提供雙 tool——`submitPlan`（通過）和
    `rejectInput`（拒絕，Input Gate FR-185~188）。解析 NL 意圖 → 選 agent config → 組裝 ExecutionPlan。
    非 NotebookLM 請求透過 `rejectInput` 拒絕，不進入 Executor 階段。
  - **Executor Session**（執行層，per step）：每個 `agents/*.md` 的 prompt 作為 systemMessage，
    帶 browser tools via `defineTool()`，只帶該 step 需要的 tool 子集。
    每個 executor = 一個 focused execution unit（= spike 的 production 版本）：
...
- Q: `open_notebook` 與 `register_notebook` 的語意區分？ → A: _(已過時——Review Point 1.5 決議砍掉 `open_notebook`/`close_notebook`，見 FR-175。)_ `register_notebook` tool（url, alias）納管既有 notebook（使用者已知 URL）；建立全新 notebook 透過 `exec` tool 自然語言完成（URL 由 NotebookLM 在瀏覽器中動態產生，使用者事先不知道）。Tab 由系統自動管理（tab pool acquire/release），使用者不需手動管理。
...
- Q: 為什麼用 Two-Session（Planner+Executor）而不是 spike 的扁平模式？ → A: Spike 成功因為 prompt 只做一件事。如果把所有操作（create-notebook, add-source, query, rename, delete, sync...）寫在同一份 session prompt 裡，prompt 太肥且 agent 會 lost focus。Two-Session 架構：Planner Session（輕量路由 + Input Gate，無 browser tools）解析意圖選 agent；Executor Session（per step）帶 focused prompt + KNOWLEDGE + task tools 執行。原本設計用 CustomAgent sub-agent，但 Finding #39 發現 sub-agent 無法存取 `defineTool()` custom tools，改為獨立 session。
...
- Q: Executor 怎麼知道自己在哪個頁面？ → A: 系統用 `tab.url` exact match 做 O(1) 錨點判斷（根據 AgentConfig.startPage 決定目標是 homepage 或 notebook URL），必要時先 navigate。結果作為 **hint** 注入 Executor prompt，不是 assertion——程式判斷可能出錯（redirect、session 過期等），agent 保留完整自主權可自行觀測確認。Prompt 寫法：「系統檢查：目前 tab URL 符合 X」而非「你在 X」。
