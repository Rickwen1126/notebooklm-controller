...
**Independent Test**: add_notebook 納管兩個 notebook，同時對兩個 notebook 發出 exec 操作，
驗證 parallel 執行（各自取得獨立 tab）。list_notebooks 列出所有。remove_notebook 移除一個。
另外：使用 add_all_notebooks tool 批次納管。
...
- Q: `open_notebook` 與 `add_notebook` 的語意區分？ → A: _(已過時——Review Point 1.5 決議砍掉 `open_notebook`/`close_notebook`，見 FR-175。)_ `add_notebook` tool（url, alias）納管既有 notebook（使用者已知 URL）；建立全新 notebook 透過 `exec` tool 自然語言完成（URL 由 NotebookLM 在瀏覽器中動態產生，使用者事先不知道）。Tab 由系統自動管理（tab pool acquire/release），使用者不需手動管理。
