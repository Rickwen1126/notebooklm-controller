...
40. **架構轉向**：放棄 `customAgents` sub-agent + MCP self-loop，改為 Two-Session Planner+Executor。
...
## Phase F — Two-Session Planner+Executor (session 6)

### 概要

驗證雙 session 架構：Planner（無 browser tools，純意圖解析 + prompt 組裝）→ Executor（browser tools，執行操作）。

### 結果

| 測試 | Prompt | Planner | Executor | Total |
|------|--------|---------|----------|-------|
| 單步 | "列出來源" | 11s, 2 calls | 22.8s, 6 calls | 33.8s |
| 複合 | "問問題 + 列來源" | 14.6s, 2 calls | 72.5s + 24s | 111.1s |

### Batch Test — 13/13 全操作通過

| ID | 操作 | 狀態 | 耗時 |
|----|------|------|------|
| T01 | list-sources | PASS | 38s |
| T02 | sync-notebook | PASS | 175s |
| T03 | query | PASS | 79s |
| T04 | add-source (text) | PASS | 66s |
| T05 | list-sources (驗證) | PASS | 41s |
| T06 | rename-source | PASS | 93s |
| T07 | query (cross-source) | PASS | 65s |
| T08 | clear-chat | PASS | 46s |
| T09 | remove-source | PASS | 62s |
| T10 | list-sources (驗證) | PASS | 22s |
| T11 | create notebook | PASS | 150s |
| T12 | rename notebook | PASS | 82s |
| T13 | delete notebook | PASS | 60s |

### Key findings (#41-#46)

41. **Two-Session Planner+Executor 完全可行** — Planner 正確拆解意圖（單步/複合），Executor 用 `defineTool()` custom tools 完成操作。架構簡潔：兩個 `createSession → sendAndWait → disconnect`，中間純資料傳遞。
42. **Built-in tools 殘留** — Executor session 仍可呼叫 bash/view，偶爾 fallback。生產環境需在 systemMessage 限制或等 SDK `disableBuiltInTools`。
43. **CDP Ctrl+A 在 Angular Material dialog 失效** — 必須用 JS `document.activeElement.select()` 取代 CDP key event。
44. **Dialog 確認步驟必須明確** — agent prompt 的 dialog 處理不能寫「→ 確認」帶過，必須寫出 `find("按鈕文字") → click`。GPT-4.1 非推理模型無法自行推斷。
45. **不需要 reasoning model** — Planner（分類）+ Executor（照做）都用 GPT-4.1 足夠。Planner 13/13 選對 agent。prompt 品質 > 模型能力。
46. **任務完成驗證用截圖** — 截圖 + 預期畫面描述是最直接的客觀驗證，不需結構化 reportResult tool。
